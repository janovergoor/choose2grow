...
```{r data}
d_mat = read_csv("~/choosing_to_grow/data_academic/processed/mag_cli.txt", col_types='cccddcc')
...
edges %>%
  ...
  sample_n(100) %>%
  left_join(d_mat %>% select(id, authors, year, lang)) %>%
  select(paper_id=id, paper_year=year, paper_authors=authors, paper_lang=lang, choice_id=cites, choice_year=year_cited) 
```

```{r, eval=F}
library(mlogit)
d = read_csv("~/projects/choosing_to_grow/data/choices_sampled/g-0.50-1.00-00.csv") %>%
  group_by(choice_id) %>% mutate(alt_id = row_number()) %>% mutate(has_fof=fof>0)
H <- mlogit.data(d, shape="long", choice="y", alt.var='alt_id', varying=c(3:4,6))
m <- mlogit(y ~ log(deg) + has_fof | 0, H)
summary(m)
```


```{r}
list.files("~/projects/choosing_to_grow/data/choices_sampled/")
