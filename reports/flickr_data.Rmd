...
```{r model_models1}
fs <- list(
  mlogit(y ~ safe_log(deg) + has_deg | 0, H_train),
  mlogit(y ~ safe_log(deg) + has_deg + recip | 0, H_train),
  mlogit(y ~                           recip + is_fof | 0, H_train),
  mlogit(y ~ safe_log(deg) + has_deg + recip + is_fof | 0, H_train),
  mlogit(y ~ safe_log(deg) + has_deg + recip + hops_f | 0, H_train),
  # mlogit(y ~ safe_log(deg) + has_deg + recip + safe_log(hops) | 0, H_train2),
  mlogit(y ~ safe_log(deg) + has_deg + recip + safe_log(n_paths) | 0, H_train)
)
```
...
```{r np_write}
H_train3 <- H_train %>%
  mutate(deg = ifelse(deg > 300, 300, deg)) %>%
...
DF <- rbind(
  ...
  data.frame(deg='alpha', est=coef(fs[[2]])[1], se=coef(summary(fs[[2]]))[1,2], model='1'),
  data.frame(deg=1:130  , est=coef(f3)[1:130] , se=coef(summary(f3))[1:130,2] , model='3'),
  data.frame(deg='alpha', est=coef(fs[[4]])[1], se=coef(summary(fs[[4]]))[1,2], model='3'),
  data.frame(deg=1:130  , est=coef(f4)[1:130] , se=coef(summary(f4))[1:130,2] , model='4'),
  data.frame(deg='alpha', est=coef(fs[[5]])[1], se=coef(summary(fs[[5]]))[1,2], model='4')
)

write_csv(DF, "../results/fig6_flickr.csv")
```
