...
H_train <- read.csv("../results/mag_cli_H_train.csv") %>%
  mutate(max_n_papers=ifelse(is.na(max_n_papers), 0, max_n_papers)) %>%
  mutate(n_citations_f = ifelse(n_citations > 130, 130, n_citations)) %>%
...
DF <- rbind(
  data.frame(deg=1:130  , est=coef(f1)[1:130] , se=coef(summary(f1))[1:130,2] , model='1'),
  data.frame(deg='alpha', est=coef(fs[[1]])[1], se=coef(summary(fs[[1]]))[1,2], model='1'),
  data.frame(deg=1:130  , est=coef(f3)[1:130] , se=coef(summary(f3))[1:130,2] , model='3'),
