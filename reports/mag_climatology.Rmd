...
R -e "rmarkdown::render('mag_climatology.Rmd', output_file='mag_climatology.pdf')"
```
...
* total number of papers after 1950 (=nodes): `r nrow(d_raw %>% filter(year>150))`
* total number of references: `r nrow(edges %>% filter(!is.na(cites), year_cited > 150))`
* total number of references to known nodes (=edges): `r nrow(edges %>% filter(!is.na(year_published), year_cited > 150))`
* share of references to known nodes: `r edges %>% filter(!is.na(cites), year_cited > 150) %>% summarize(mean(!is.na(year_published))) %>% as.numeric()`
* total number of papers with at least one out-link: `r nrow(edges %>% filter(!is.na(cites), year_cited > 150) %>% distinct(id))`

```{r plot_dist_time}
Rmisc::multiplot(
  # distribution of when papers are published
  d_raw %>% filter(year > 150) %>%
...
Out-degree over time (global average is `r deg_out %>% filter(year_cited > 150) %>% ungroup() %>% summarize(mean(n_out)) %>% as.numeric() %>% round(2)`):
...
* sample 10000 citations from after 2011
...
# store the data
write_csv(H_train, "../results/mag_cli_H_train.csv")
write_csv(H_test, "../results/mag_cli_H_test.csv")
```

```{r data_reload}
H_train <- read.csv("../results/mag_cli_H_train.csv") %>%
  mutate(max_n_papers=ifelse(is.na(max_n_papers), 0, max_n_papers)) %>%
  mutate(has_deg=ifelse(n_citations > 0, 1, 0)) %>%
  mlogit.data(chid.var='choice_id', choice="y", alt.var='alt_id')
H_test  <- read.csv("../results/mag_cli_H_test.csv") %>%
  mutate(max_n_papers=ifelse(is.na(max_n_papers), 0, max_n_papers)) %>%
  mutate(has_deg=ifelse(n_citations > 0, 1, 0)) %>%
  mlogit.data(chid.var='choice_id', choice="y", alt.var='alt_id')
```
...
```{r model_models}
fs <- list(
  mlogit(y ~ safe_log(n_citations) + has_deg | 0, H_train),
  mlogit(y ~ safe_log(n_citations) + has_deg + has_same_author | 0, H_train),
  mlogit(y ~ safe_log(n_citations) + has_deg + has_same_author + log(delta_years) | 0, H_train),
  mlogit(y ~ safe_log(n_citations) + has_deg + has_same_author + log(delta_years)  + safe_log(max_n_papers) | 0, H_train),
  mlogit(y ~ safe_log(n_citations) + has_deg + has_same_author + log(delta_years)  + safe_log(max_n_papers) + n_same_keywords|  0, H_train),
  mlogit(y ~ safe_log(n_citations) + has_deg + has_same_author + is_old*safe_log(n_citations) | 0, H_train)
)
```
...
H_train <- read.csv("../results/mag_cli_H_train.csv") %>%
  mutate(max_n_papers=ifelse(is.na(max_n_papers), 0, max_n_papers)) %>%
  mutate(n_citations_f = ifelse(n_citations > 100, 101, n_citations)) %>%
  group_by(choice_id) %>% filter(sum(ifelse(y, 1, 0)) == 1) %>% ungroup() %>%
  mlogit.data(chid.var='choice_id', choice="y", alt.var='alt_id')

f1 <- mlogit(y ~ as.factor(n_citations_f) | 0, H_train)
f3 <- mlogit(y ~ as.factor(n_citations_f) + has_same_author + log(delta_years) | 0, H_train)

DF <- rbind(
  data.frame(deg=1:100  , est=coef(f1)[1:100] , se=coef(summary(f1))[1:100,2] , model='1'),
  data.frame(deg='alpha', est=coef(fs[[1]])[1], se=coef(summary(fs[[1]]))[1,2], model='1'),
  data.frame(deg=1:100  , est=coef(f3)[1:100] , se=coef(summary(f3))[1:100,2] , model='3'),
  data.frame(deg='alpha', est=coef(fs[[3]])[1], se=coef(summary(fs[[3]]))[1,2], model='3')
)

write_csv(DF, "../results/fig6_citations.csv")
```

```{r np_write2}
D_test <- rbind(
    data.frame(choice_id=1, alt_id=0, y=0, n_citations=0, has_deg=0, has_same_author=0, delta_years=1),
    data.frame(choice_id=1, alt_id=1:100, y=0, n_citations=1:100, has_deg=1, has_same_author=0, delta_years=1)
  )%>%
  mlogit.data(chid.var='choice_id', choice="y", alt.var='alt_id')
...
DF <- rbind(
  ...
  data.frame(deg=0:100, est=predict(fs[[1]], D_test), model='1', type='line'),
  ...
  data.frame(deg=0:100, est=predict(fs[[3]], D_test), model='3', type='line')
)

write_csv(DF, "../results/fig6_citations2.csv")
```
