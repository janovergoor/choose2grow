...
make_data <- function(n, seed=100){
    ...
    H <- DM %>% 
      ...
      select(choice_id=paper_id, alt_id, y, n_citations:max_n_papers) %>% as.data.frame()

  return(H)
}
```

```{r actually_make_data1}
# split generation up in parts to not use too much memory at once
a <- make_data(n=2500, seed=133)
b <- make_data(n=2500, seed=549)
a <- rbind(a, b[!(b$choice_id %in% a$choice_id), ])  # join together
b <- make_data(n=1000, seed=791)  # get few extra to make sure to hit 5000
c <- rbind(a, b[!(b$choice_id %in% a$choice_id), ])  # join together
```
```{r actually_make_data2}
samples = distinct(c, choice_id) %>% sample_n(5000) %>% .$choice_id
H_train <- c %>%
...
f2 <- mlogit(y ~ log(n_citations+1) + has_same_author | 0, H_train)
f3 <- mlogit(y ~ log(n_citations+1) + has_same_author + log(delta_years) | 0, H_train)
f4 <- mlogit(y ~ log(n_citations+1) + has_same_author + log(delta_years)  + log(max_n_papers+1) | 0, H_train)
f5 <- mlogit(y ~ log(n_citations+1) + has_same_author + log(delta_years)  + log(max_n_papers+1) + n_same_keywords| 0, H_train)
```
...
stargazer::stargazer(
  f1, f2, f3, f4, f5,
  dep.var.caption = "",
  covariate.labels = c("log Citations", "Has same author", "log Age", "Max papers by author", "# same keywords"),
  header=FALSE, 
  type='text')
```
...
c(acc(f1, H_train), acc(f2, H_train), acc(f3, H_train), acc(f4, H_train), acc(f5, H_train)) %>%
  round(4) %>% paste(collapse=' & ') %>% print()
print("Test accuracy:")
c(acc(f1, H_test), acc(f2, H_test), acc(f3, H_test), acc(f4, H_test), acc(f5, H_test)) %>%
  round(4) %>% paste(collapse=' & ') %>% print()
```