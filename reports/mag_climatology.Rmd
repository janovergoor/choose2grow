...
make_data <- function(n, seed=NULL){
    ...
    DM <- dm3 %>%
      ...
      mutate(n_citations_f = factor(ifelse(n_citations > 150, 150, n_citations))) %>%
...
X <- make_data(n=100)
while(nrow(distinct(X, choice_id)) < 10000) {
  message(nrow(distinct(X, choice_id)))
  Y <- make_data(n=1000)
  X <- rbind(X, Y[!(Y$choice_id %in% X$choice_id), ])  # join together
}
message(nrow(distinct(X, choice_id)))
samples = distinct(X, choice_id) %>% sample_n(10000) %>% .$choice_id

# convert to mlogit.data
H_train <- X %>%
  filter(choice_id %in% samples) %>%
...
```{r model_models}
fs <- list(
  mlogit(y ~ log(n_citations+1) | 0, H_train),
  mlogit(y ~ log(n_citations+1) + has_same_author | 0, H_train),
  mlogit(y ~ log(n_citations+1) + has_same_author + log(delta_years) | 0, H_train),
  mlogit(y ~ log(n_citations+1) + has_same_author + log(delta_years)  + log(max_n_papers+1) | 0, H_train),
  mlogit(y ~ log(n_citations+1) + has_same_author + log(delta_years)  + log(max_n_papers+1) + n_same_keywords|  0, H_train),
  mlogit(y ~ log(n_citations+1) + has_same_author + is_old*log(n_citations+1) | 0, H_train)
)
```
...
stargazer::stargazer(
  fs,
  dep.var.caption = "",
  covariate.labels = c("log Citations", "Has same author", "log Age", "Max papers by author", "# same keywords", "+10 years", "+10 years*log Citations"),
  header=FALSE, 
  type='text')
```
...
```{r np_write}
f1 <- mlogit(y ~ n_citations_f | 0, H_train)
f2 <- mlogit(y ~ n_citations_f + has_same_author | 0, H_train)
f3 <- mlogit(y ~ n_citations_f + has_same_author + log(delta_years) | 0, H_train)
f4 <- mlogit(y ~ n_citations_f + has_same_author + log(delta_years)  + log(max_n_papers+1) | 0, H_train)

DF <- rbind(
  data.frame(deg=1:150, est=coef(f1)[1:150], model='1'),
  data.frame(deg=1:150, est=coef(f2)[1:150], model='2'),
  data.frame(deg=1:150, est=coef(f3)[1:150], model='3'),
  data.frame(deg=1:150, est=coef(f4)[1:150], model='4')
)

write_csv(DF, "~/projects/choosing_to_grow/choose2grow/data/citations-non_parametric.csv")
```