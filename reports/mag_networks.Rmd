...
make_data <- function(n, seed=100){
    ...
    H <- DM %>% 
      ...
      select(paper_id, alt_id, y, n_citations:max_n_papers) %>% as.data.frame()
...
a <- rbind(a, b[!(b$paper_id %in% a$paper_id), ])  # join together
b <- make_data(n=1000, seed=791)  # get few extra to make sure to hit 5000
c <- rbind(a, b[!(b$paper_id %in% a$paper_id), ])  # join together
```
```{r actually_make_data2}
samples = distinct(c, paper_id) %>% sample_n(5000) %>% .$paper_id
H_train <- c %>%
  filter(paper_id %in% samples) %>%
  mlogit.data(chid.var='paper_id', choice="y", alt.var='alt_id')
...
print(c(acc(f1, H_train), acc(f2, H_train), acc(f3, H_train), acc(f4, H_train), acc(f5, H_train)))
print("Test accuracy:")
print(c(acc(f1, H_test), acc(f2, H_test), acc(f3, H_test), acc(f4, H_test), acc(f5, H_test)))
```